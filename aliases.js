...
if (window.rcmail) {
	...
	var tab = $('<span>').attr('id', 'settingstabpluginaliases').addClass('tablink aliases');
	var button = $('<a>').attr('href', rcmail.env.comm_path+'&_action=plugin.aliases')
	    .attr('title', rcmail.gettext('aliases.aliasesmanagealiases'))
	    .html(rcmail.gettext('aliases.aliasesmanagealiases')).appendTo(tab);
